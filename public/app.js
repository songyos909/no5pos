...
function modifierExtra(options) { return (options.toppings.includes('extraShot') ? 15 : 0) + (options.toppings.includes('whippedCream') ? 10 : 0); }
function modifierSummary(options) { const labels={hot:'ร้อน',iced:'เย็น',blended:'ปั่น',fresh:'นมสด',extraShot:'เพิ่มช็อต',whippedCream:'วิปครีม'}; return [labels[options.temperature],`หวาน ${options.sweetness}%`,labels[options.milk],...options.toppings.map(x=>labels[x])].join(' · '); }

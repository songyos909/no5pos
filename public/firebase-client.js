...
(function () {
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='bootstrap' && method==='GET') { const [products,inventory,categories,channels,optionGroups,settings]=await Promise.all([docs('products'),docs('inventory'),docs('categories'),docs('channels'),docs('optionGroups'),db.collection('settings').doc('features').get()]); const categoryRows=categories.length?categories.filter(x=>x.active!==false).map(x=>({category_key:x.id,...x})):defaults.categories.map(([category_key,name])=>({category_key,name,active:true}));const channelRows=channels.length?channels.filter(x=>x.active!==false).map(x=>({channel_key:x.id,...x})):defaults.channels.map(([channel_key,name,gp_percent])=>({channel_key,name,gp_percent,active:true}));return {products:products.filter(x=>x.active!==false).map(x=>({id:Number(x.id)||x.id,...localized(x)})),inventory:inventory.map(x=>({stock_key:x.id,...localized(x)})),categories:categoryRows,channels:channelRows,optionGroups,features:settings.data()||defaults.features,membersEnabled:true}; }
    ...
    if(path==='reports/transactions' && method==='GET') { const [orders,items]=await Promise.all([docs('orders'),docs('orderItems')]);return orders.sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).slice(0,50).map(o=>({id:o.id,created_at:o.createdAt,subtotal:Number(o.subtotal||0),discount:Number(o.discount||0),total:Number(o.total||0),payment_type:o.paymentType||o.payment_type||'cash',sales_channel:o.salesChannel||'store',online_platform:o.onlinePlatform||null,gp_percent:Number(o.gpPercent||0),online_net:Number(o.onlineNet??o.total??0),member_phone:o.memberPhone||null,received:Number(o.received||0),change_due:Number(o.changeDue||0),items:items.filter(x=>x.order_id===o.id).map(x=>({name:x.name,product_id:x.product_id||null,unit_price:Number(x.unit_price||0),quantity:Number(x.quantity||0),options_json:x.options_json||'{}'}))})); }
    if(path==='reports/analytics' && method==='GET') { const [orders,items,products]=await Promise.all([docs('orders'),docs('orderItems'),docs('products')]);const byId=Object.fromEntries(products.map(p=>[String(p.id),p])),byName=Object.fromEntries(products.map(p=>[p.name,p]));const categoryMap={},sellerMap={},paymentMap={};items.forEach(x=>{const revenue=Number(x.unit_price||0)*Number(x.quantity||0),product=byId[String(x.product_id)]||byName[x.name],category=product?.category||'other';categoryMap[category]=(categoryMap[category]||0)+revenue;const seller=sellerMap[x.name]||{name:x.name,qty:0,revenue:0};seller.qty+=Number(x.quantity||0);seller.revenue+=revenue;sellerMap[x.name]=seller;});orders.forEach(o=>{const key=o.paymentType||o.payment_type||'cash';paymentMap[key]=(paymentMap[key]||0)+Number(o.total||0);});return {categorySales:Object.entries(categoryMap).map(([category,sales])=>({category,sales})),paymentSales:Object.entries(paymentMap).map(([payment_type,sales])=>({payment_type,sales})),topSellers:Object.values(sellerMap).sort((a,b)=>b.qty-a.qty).slice(0,5)}; }
    ...
    if(path==='orders' && method==='POST') {
      const [allProducts,recipeItems]=await Promise.all([docs('products'),docs('recipeItems')]);
      const products=Object.fromEntries(allProducts.map(p=>[String(p.id),p]));
      const lines=(data.items||[]).map(i=>{
        const p=products[String(i.productId)];
        if(!p)return err('ไม่พบสินค้า');
        ...
        return {product:p,quantity:Number(i.quantity),options:o,unit_price:Number(p.price)+customExtra};
      });
      if(!lines.length)return err('ไม่มีสินค้า');
      const stockLines=lines.filter(line=>line.product.deduct_stock===true);
      ...
      const subtotal=lines.reduce((n,x)=>n+x.quantity*x.unit_price,0),discount=Math.min(Number(data.discount)||0,subtotal),total=subtotal-discount,id=uid(),createdAt=new Date().toISOString(),salesChannel=data.salesChannel==='online'?'online':'store',onlinePlatform=salesChannel==='online'?String(data.onlinePlatform||''):null,gpPercent=salesChannel==='online'?Number(data.gpPercent||0):0,onlineNet=salesChannel==='online'?Number((total*(1-gpPercent/100)).toFixed(2)):total;
      if(salesChannel==='online'&&(!onlinePlatform||gpPercent<=0||gpPercent>=100))return err('กรุณาตั้งค่า GP จริงของแพลตฟอร์ม');
      const used={};
      ...
      await db.runTransaction(async tx=>{
        for(const [key,amount] of Object.entries(used)){const ref=db.collection('inventory').doc(key),snap=await tx.get(ref);if(!snap.exists||Number(snap.data().quantity||0)<amount)err(`สต็อกไม่พอ: ${snap.data()?.name||key}`);tx.update(ref,{quantity:Number(snap.data().quantity)-amount});}
        tx.set(db.collection('orders').doc(id),{id,subtotal,discount,total,paymentType:data.paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,memberPhone:data.memberPhone||null,received:data.received||0,changeDue:data.changeDue||0,createdAt});
        ...
        if(data.memberPhone){const ref=db.collection('members').doc(String(data.memberPhone));const snap=await tx.get(ref);if(snap.exists){const oldPoints=Number(snap.data().points||0);if(data.redeemFreeCup&&oldPoints<10)err('จำนวนแก้วสะสมไม่เพียงพอสำหรับแลกฟรี');const earned=Math.max(0,beverageCups-(data.redeemFreeCup?1:0));memberPoints=oldPoints-(data.redeemFreeCup?10:0)+earned;tx.update(ref,{points:memberPoints});}}
      });
      return {id,subtotal,discount,total,paymentType:data.paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,memberPhone:data.memberPhone,memberPoints,received:data.received||0,changeDue:data.changeDue||0,createdAt,items:lines.map(x=>({name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options:x.options}))};
    }
    ...
    if(path==='admin/option-groups' && method==='POST') { const id=String(data.id||Date.now());const group={id,name:String(data.name||'').trim(),choices:Array.isArray(data.choices)?data.choices:[]};if(!group.name||!group.choices.length)return err('ข้อมูลตัวเลือกไม่ถูกต้อง');await db.collection('optionGroups').doc(id).set(group);return group; }
    if(path.match(/^admin\/option-groups\/[^/]+$/) && method==='PUT') { const id=decodeURIComponent(path.split('/')[2]),group={id,name:String(data.name||'').trim(),choices:Array.isArray(data.choices)?data.choices:[]};if(!group.name||!group.choices.length)return err('ข้อมูลตัวเลือกไม่ถูกต้อง');await db.collection('optionGroups').doc(id).set(group);await syncFirebaseOptionGroup(id,group);return group; }
    ...
    if(path==='admin/products' && method==='POST') { const id=String(Date.now());await db.collection('products').doc(id).set({id,name:data.name,price:Number(data.price),category:data.category||'other',emoji:data.emoji||'☕',image_path:data.imagePath||null,image_data:data.imageData||null,custom_options:Array.isArray(data.customOptions)?data.customOptions:[],deduct_stock:data.deductStock!==false,active:true,target_margin:.65});return {id}; }
    ...
    if(path==='costing' && method==='GET') { const [products,items,inventory,channels]=await Promise.all([docs('products'),docs('recipeItems'),docs('inventory'),docs('channels')]);const effectiveChannels=channels.length?channels:defaults.channels.map(([id,name,gp_percent])=>({id,name,gp_percent,active:true}));const inv=Object.fromEntries(inventory.map(x=>[x.id,localized(x)]));return products.filter(x=>x.active!==false).map(p=>{const ingredients=items.filter(x=>String(x.product_id)===String(p.id)).map(x=>({...x,name:inv[x.stock_key]?.name||x.stock_key,unit:inv[x.stock_key]?.unit||'',cost_per_unit:Number(inv[x.stock_key]?.cost_per_unit||0),line_cost:Number((Number(x.quantity)*Number(inv[x.stock_key]?.cost_per_unit||0)).toFixed(2))}));const cost=ingredients.reduce((n,x)=>n+x.line_cost,0),target_margin=Number(p.target_margin??.65),recommended_store_price=cost/(1-target_margin);return {product_id:p.id,name:localized(p).name,store_price:Number(p.price),target_margin,cost,food_cost_percent:p.price?Number((cost/p.price*100).toFixed(1)):0,gross_profit:Number(p.price)-cost,recommended_store_price,ingredients,online:effectiveChannels.filter(x=>x.active!==false).map(c=>({channel_key:c.id,name:c.name,gp_percent:c.gp_percent,suggested_price:recommended_store_price/(1-Number(c.gp_percent)/100)}))};}); }
    if(path==='pricing' && method==='GET') { const [products,channels,prices]=await Promise.all([docs('products'),docs('channels'),docs('channelPrices')]);const effectiveChannels=channels.length?channels:defaults.channels.map(([id,name,gp_percent])=>({id,name,gp_percent,active:true}));return products.filter(p=>p.active!==false).flatMap(p=>effectiveChannels.filter(c=>c.active!==false).map(c=>{const saved=prices.find(x=>String(x.product_id)===String(p.id)&&x.channel_key===c.id);return {product_id:p.id,name:localized(p).name,store_price:Number(p.price),channel_key:c.id,channel_name:c.name,gp_percent:Number(c.gp_percent),sale_price:saved?.sale_price??null,suggested_price:Number((Number(p.price)/(1-Number(c.gp_percent)/100)).toFixed(2))};})); }
    ...
    if(path==='admin/cost-inventory' && method==='POST') { const key=String(data.stockKey||'').trim();if(!key||!data.name||!data.unit)return err('กรอกข้อมูลวัตถุดิบให้ครบ');const purchaseQuantity=Number(data.purchaseQuantity),purchaseTotal=Number(data.purchaseTotal);await db.collection('inventory').doc(key).set({name:data.name,unit:data.unit,quantity:Number(data.quantity)||0,low_alert:Number(data.lowAlert)||0,category:data.category==='equipment'?'equipment':'ingredient',material_type:data.materialType||'other',purchase_quantity:purchaseQuantity,purchase_total:purchaseTotal,cost_per_unit:purchaseQuantity?purchaseTotal/purchaseQuantity:0});return {ok:true}; }
    if(path.match(/^admin\/cost-inventory\/[^/]+$/) && method==='PUT') { const key=path.split('/')[2],purchaseQuantity=Number(data.purchaseQuantity),purchaseTotal=Number(data.purchaseTotal);await db.collection('inventory').doc(key).set({name:data.name,unit:data.unit,quantity:Number(data.quantity)||0,low_alert:Number(data.lowAlert)||0,category:data.category==='equipment'?'equipment':'ingredient',material_type:data.materialType||'other',purchase_quantity:purchaseQuantity,purchase_total:purchaseTotal,cost_per_unit:purchaseQuantity?purchaseTotal/purchaseQuantity:0},{merge:true});return {ok:true}; }
    if(path.match(/^admin\/inventory\/[^/]+\/adjust$/) && method==='POST') { const ref=db.collection('inventory').doc(path.split('/')[2]),amount=Number(data.amount);await db.runTransaction(async tx=>{const snap=await tx.get(ref);if(!snap.exists||Number(snap.data().quantity||0)+amount<0)err('สต็อกไม่พอหรือไม่พบรายการ');tx.update(ref,{quantity:Number(snap.data().quantity||0)+amount});});return {ok:true}; }
    if(path.match(/^admin\/inventory\/[^/]+$/) && method==='DELETE') { await db.collection('inventory').doc(path.split('/')[2]).delete();return {ok:true}; }
    ...
    if(path.match(/^admin\/members\/[^/]+$/) && method==='PUT') { await db.collection('members').doc(path.split('/')[2]).set({name:data.name,points:Number(data.points)||0},{merge:true});return {ok:true}; }
    if(path.match(/^admin\/members\/[^/]+$/) && method==='DELETE') { await db.collection('members').doc(path.split('/')[2]).delete();return {ok:true}; }
    if(path==='admin/categories' && method==='POST') { const key=String(data.key||'').trim();if(!key||!data.name)return err('กรอกหมวดหมู่ให้ครบ');await db.collection('categories').doc(key).set({name:data.name,active:true});return {ok:true}; }
    if(path.match(/^admin\/categories\/[^/]+$/) && method==='PUT') { await db.collection('categories').doc(path.split('/')[2]).set({name:data.name},{merge:true});return {ok:true}; }
    if(path.match(/^admin\/categories\/[^/]+$/) && method==='DELETE') { await db.collection('categories').doc(path.split('/')[2]).delete();return {ok:true}; }
    if(path.match(/^admin\/channels\/[^/]+$/) && method==='PUT') { await db.collection('channels').doc(path.split('/')[2]).set({gp_percent:Number(data.gpPercent),active:data.active!==false},{merge:true});return {ok:true}; }
    if(path==='admin/channel-prices' && method==='PUT') { const id=`${data.productId}_${data.channelKey}`;await db.collection('channelPrices').doc(id).set({product_id:data.productId,channel_key:data.channelKey,sale_price:Number(data.salePrice)},{merge:true});return {ok:true}; }

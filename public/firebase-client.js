...
(function () {
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='bootstrap' && method==='GET') { await Promise.all([restoreDefaultCategories(),restoreDefaultChannels()]);const [products,inventory,categories,channels,channelPrices,optionGroups,orderItems,settings]=await Promise.all([docs('products'),docs('inventory'),docs('categories'),docs('channels'),docs('channelPrices'),docs('optionGroups'),docs('orderItems'),db.collection('settings').doc('features').get()]); const categoryRows=effectiveFirebaseCategories(categories).map(x=>({category_key:x.id,...x}));const channelRows=effectiveFirebaseChannels(channels).map(x=>({channel_key:x.id,...x}));const features=Object.fromEntries(Object.entries(settings.data()||defaults.features).filter(([key])=>Object.hasOwn(defaults.features,key)));const activeIds=new Set(products.filter(x=>x.active!==false).map(x=>String(x.id))),sellerTotals={};orderItems.forEach(item=>{const id=String(item.product_id??'');if(activeIds.has(id))sellerTotals[id]=(sellerTotals[id]||0)+Number(item.quantity||0);});const bestSellers=Object.entries(sellerTotals).map(([product_id,qty])=>({product_id,qty})).sort((a,b)=>b.qty-a.qty||String(a.product_id).localeCompare(String(b.product_id))).slice(0,10);return {products:sortBySavedOrder(products.filter(x=>x.active!==false)).map(x=>({id:Number(x.id)||x.id,...localized(x)})),inventory:inventory.map(x=>({stock_key:x.id,...localized(x)})),categories:categoryRows,channels:channelRows,channelPrices,optionGroups:sortBySavedOrder(optionGroups),bestSellers,features,membersEnabled:true}; }
    ...
    if(path==='orders' && method==='POST') {
      ...
      return {id,subtotal,discount,total,paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,memberPhone:data.memberPhone,memberPoints,received,changeDue,createdAt,items:lines.map(x=>({name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options:x.options}))};
    }

...
(function () {
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='orders' && method==='POST') { const [allProducts,recipeItems]=await Promise.all([docs('products'),docs('recipeItems')]);const products=Object.fromEntries(allProducts.map(p=>[String(p.id),p]));const lines=(data.items||[]).map(i=>{const p=products[String(i.productId)];if(!p)return err('ไม่พบสินค้า');const o={...(i.options||{}),milk:'fresh',toppings:(i.options?.toppings||[]).filter(x=>x==='extraShot')};const plus=o.toppings.includes('extraShot')?20:0;return {product:p,quantity:Number(i.quantity),options:o,unit_price:Number(p.price)+plus};});if(!lines.length)return err('ไม่มีสินค้า');const stockLines=lines.filter(line=>line.product.deduct_stock===true);const missing=stockLines.find(line=>!recipeItems.some(r=>String(r.product_id)===String(line.product.id)));if(missing)return err(`เมนู ${localized(missing.product).name} ยังไม่มีสูตรชง หรือเลือก “ไม่ตัด stock” ในหน้าจัดการเมนู`);const subtotal=lines.reduce((n,x)=>n+x.quantity*x.unit_price,0),discount=Math.min(Number(data.discount)||0,subtotal),total=subtotal-discount,id=uid(),createdAt=new Date().toISOString();const used={};stockLines.forEach(line=>recipeItems.filter(r=>String(r.product_id)===String(line.product.id)).forEach(r=>used[r.stock_key]=(used[r.stock_key]||0)+Number(r.quantity)*line.quantity));await db.runTransaction(async tx=>{for(const [key,amount] of Object.entries(used)){const ref=db.collection('inventory').doc(key),snap=await tx.get(ref);if(!snap.exists||Number(snap.data().quantity||0)<amount)err(`สต็อกไม่พอ: ${snap.data()?.name||key}`);tx.update(ref,{quantity:Number(snap.data().quantity)-amount});}tx.set(db.collection('orders').doc(id),{id,subtotal,discount,total,paymentType:data.paymentType,memberPhone:data.memberPhone||null,received:data.received||0,changeDue:data.changeDue||0,createdAt});lines.forEach(x=>tx.set(db.collection('orderItems').doc(),{order_id:id,product_id:String(x.product.id),name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options_json:JSON.stringify(x.options),status:'pending',created_at:createdAt,createdAt}));if(data.memberPhone){const ref=db.collection('members').doc(String(data.memberPhone));const snap=await tx.get(ref);if(snap.exists)tx.update(ref,{points:firebase.firestore.FieldValue.increment(Math.floor(total/10))});}});return {id,subtotal,discount,total,paymentType:data.paymentType,memberPhone:data.memberPhone,received:data.received||0,changeDue:data.changeDue||0,createdAt,items:lines.map(x=>({name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options:x.options}))}; }

...
(function () {
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='pricing' && method==='GET') { const [products,channels,prices]=await Promise.all([docs('products'),docs('channels'),docs('channelPrices')]);return products.filter(p=>p.active!==false).flatMap(p=>channels.filter(c=>c.active!==false).map(c=>{const saved=prices.find(x=>String(x.product_id)===String(p.id)&&x.channel_key===c.id);return {product_id:p.id,name:p.name,store_price:Number(p.price),channel_key:c.id,channel_name:c.name,gp_percent:Number(c.gp_percent),sale_price:saved?.sale_price??null,suggested_price:Number((Number(p.price)/(1-Number(c.gp_percent)/100)).toFixed(2))};})); }

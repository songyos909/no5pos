...
(function () {
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='orders' && method==='POST') {
      if(!Array.isArray(data.items)||!data.items.length||!['cash','qr'].includes(data.paymentType))return err('ข้อมูลการชำระเงินไม่ถูกต้อง');
      ...
      const received=data.paymentType==='cash'?Number(data.received):total;
      if(!Number.isFinite(received)||received<total)return err('ยอดเงินที่รับไม่เพียงพอ');
      const changeDue=data.paymentType==='cash'?Number((received-total).toFixed(2)):0;
      let memberPoints=0;
      await db.runTransaction(async tx=>{
        ...
        if(data.redeemFreeCup&&(!memberSnap?.exists||Number(memberSnap.data().points||0)<10))err('จำนวนแก้วสะสมไม่เพียงพอสำหรับแลกฟรี');
        inventoryReads.forEach(({amount,ref,snap})=>tx.update(ref,{quantity:Number(snap.data().quantity)-amount}));
        tx.set(db.collection('orders').doc(id),{id,subtotal,discount,total,paymentType:data.paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,memberPhone:data.memberPhone||null,received,changeDue,createdAt});
        ...
        if(memberSnap?.exists){const oldPoints=Number(memberSnap.data().points||0),earned=Math.max(0,beverageCups-(data.redeemFreeCup?1:0));memberPoints=oldPoints-(data.redeemFreeCup?10:0)+earned;tx.update(memberRef,{points:memberPoints});}
      });
      return {id,subtotal,discount,total,paymentType:data.paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,memberPhone:data.memberPhone,memberPoints,received,changeDue,createdAt,items:lines.map(x=>({name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options:x.options}))};
    }

...
(function () {
  ...
  const normalizeFirebaseLoyalty = raw => ({mode:['all','category','product'].includes(raw?.mode)?raw.mode:'category',categoryKeys:[...new Set((Array.isArray(raw?.categoryKeys)?raw.categoryKeys:['coffee','tea']).map(String))],productIds:[...new Set((Array.isArray(raw?.productIds)?raw.productIds:[]).map(String))],earnStore:raw?.earnStore!==false,earnOnline:raw?.earnOnline!==false,rewardPoints:Math.min(999,Math.max(1,Math.round(Number(raw?.rewardPoints)||10))),rewardType:['free_product','fixed_discount'].includes(raw?.rewardType)?raw.rewardType:'free_product',rewardMode:['all','category','product'].includes(raw?.rewardMode)?raw.rewardMode:'category',rewardCategoryKeys:[...new Set((Array.isArray(raw?.rewardCategoryKeys)?raw.rewardCategoryKeys:['coffee','tea']).map(String))],rewardProductIds:[...new Set((Array.isArray(raw?.rewardProductIds)?raw.rewardProductIds:[]).map(String))],rewardDiscountAmount:Math.max(1,Number(raw?.rewardDiscountAmount)||50),rewardMaxPrice:Math.max(0,Number(raw?.rewardMaxPrice)||0)});
  const normalizeDiscountPresets = values => [...new Set((Array.isArray(values)?values:[]).map(Number).filter(value=>Number.isFinite(value)&&value>0))].sort((a,b)=>a-b).slice(0,20);
  ...
  async function activate(user) {
    ...
    if (settingsSnap.data()?.required_menu_seeded !== true) {
      ...
      batch.set(settings, { required_menu_seeded:true }, { merge:true });
      await batch.commit();
    }
  ...
  const effectiveFirebaseCategories = categories => sortBySavedOrder(categories.filter(category=>category.active!==false));
  const effectiveFirebaseChannels = channels => channels.filter(channel=>channel.active!==false);
  ...
  const thaiDayRange = value => {
    const day=thaiDay(value),start=new Date(`${day}T00:00:00+07:00`),end=new Date(start.getTime()+86400000);
  ...
  window.firebaseApi = async (url,opts={}) => {
    ...
    if(path==='bootstrap' && method==='GET') { const [products,inventory,categories,channels,channelPrices,optionGroups,orderItems,settings,loyalty,discountSettings]=await Promise.all([docs('products'),docs('inventory'),docs('categories'),docs('channels'),docs('channelPrices'),docs('optionGroups'),db.collection('orderItems').orderBy('created_at','desc').limit(500).get().then(snapshot=>snapshot.docs.map(d=>({id:d.id,...d.data()}))),db.collection('settings').doc('features').get(),db.collection('settings').doc('loyalty').get(),db.collection('settings').doc('discountPresets').get()]); const categoryRows=effectiveFirebaseCategories(categories).map(x=>({category_key:x.id,...x}));const channelRows=effectiveFirebaseChannels(channels).map(x=>({channel_key:x.id,...x}));const features=Object.fromEntries(Object.entries(settings.data()||defaults.features).filter(([key])=>Object.hasOwn(defaults.features,key)));const activeIds=new Set(products.filter(x=>x.active!==false).map(x=>String(x.id))),sellerTotals={};orderItems.forEach(item=>{const id=String(item.product_id??'');if(activeIds.has(id))sellerTotals[id]=(sellerTotals[id]||0)+Number(item.quantity||0);});const bestSellers=Object.entries(sellerTotals).map(([product_id,qty])=>({product_id,qty})).sort((a,b)=>b.qty-a.qty||String(a.product_id).localeCompare(String(b.product_id))).slice(0,10),rawLoyalty=loyalty.data()||{},loyaltySettings={mode:['all','category','product'].includes(rawLoyalty.mode)?rawLoyalty.mode:'category',categoryKeys:Array.isArray(rawLoyalty.categoryKeys)?rawLoyalty.categoryKeys.map(String):['coffee','tea'],productIds:Array.isArray(rawLoyalty.productIds)?rawLoyalty.productIds.map(String):[],earnStore:rawLoyalty.earnStore!==false,earnOnline:rawLoyalty.earnOnline!==false,rewardPoints:Math.min(999,Math.max(1,Math.round(Number(rawLoyalty.rewardPoints)||10))),rewardType:['free_product','fixed_discount'].includes(rawLoyalty.rewardType)?rawLoyalty.rewardType:'free_product',rewardMode:['all','category','product'].includes(rawLoyalty.rewardMode)?rawLoyalty.rewardMode:'category',rewardCategoryKeys:Array.isArray(rawLoyalty.rewardCategoryKeys)?rawLoyalty.rewardCategoryKeys.map(String):['coffee','tea'],rewardProductIds:Array.isArray(rawLoyalty.rewardProductIds)?rawLoyalty.rewardProductIds.map(String):[],rewardDiscountAmount:Math.max(1,Number(rawLoyalty.rewardDiscountAmount)||50),rewardMaxPrice:Math.max(0,Number(rawLoyalty.rewardMaxPrice)||0)};return {products:sortBySavedOrder(products.filter(x=>x.active!==false)).map(x=>({id:Number(x.id)||x.id,...localized(x)})),inventory:inventory.map(x=>({stock_key:x.id,...localized(x)})),categories:categoryRows,channels:channelRows,channelPrices,optionGroups:sortBySavedOrder(optionGroups),bestSellers,discountPresets:normalizeDiscountPresets(discountSettings.data()?.values),loyaltySettings,features,membersEnabled:true}; }
    if(path==='discount-presets' && method==='PUT') { const values=normalizeDiscountPresets(data.values);await db.collection('settings').doc('discountPresets').set({values},{merge:false});return {ok:true,values}; }
    ...
    if(path==='reports/today' && method==='GET') { const range=thaiDayRange(Date.now()),today=(await db.collection('orders').where('createdAt','>=',range.start).where('createdAt','<',range.end).get()).docs.map(d=>({id:d.id,...d.data()})),store=today.filter(x=>(x.salesChannel||'store')!=='online'),online=today.filter(x=>x.salesChannel==='online');return {orders:store.length,sales:store.reduce((n,x)=>n+Number(x.total||0),0),storeOrders:store.length,storeSales:store.reduce((n,x)=>n+Number(x.total||0),0),onlineOrders:online.length,onlineSales:online.reduce((n,x)=>n+Number(x.total||0),0),onlineNet:online.reduce((n,x)=>n+Number(x.onlineNet??(Number(x.total||0)*(1-Number(x.gpPercent||0)/100))),0)}; }
    if(path==='reports/transactions' && method==='GET') { const [orders,items,products]=await Promise.all([docs('orders'),docs('orderItems'),docs('products')]),filters=firebaseReportFilters(query),byId=Object.fromEntries(products.map(p=>[String(p.id),p])),lineFiltered=!!(filters.productId||filters.category);const itemMatches=x=>(!filters.productId||String(x.product_id)===filters.productId)&&(!filters.category||(byId[String(x.product_id)]?.category||'other')===filters.category);const eligible=orders.filter(o=>firebaseOrderMatches(o,filters)&&(!lineFiltered||items.some(x=>x.order_id===o.id&&itemMatches(x))));return eligible.sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).slice(0,200).map(o=>{const visibleItems=items.filter(x=>x.order_id===o.id&&(!lineFiltered||itemMatches(x))).map(x=>({name:x.name,product_id:x.product_id||null,unit_price:Number(x.unit_price||0),quantity:Number(x.quantity||0),options_json:x.options_json||'{}'})),filteredSubtotal=visibleItems.reduce((sum,x)=>sum+x.unit_price*x.quantity,0),subtotal=lineFiltered?filteredSubtotal:Number(o.subtotal||0),discount=lineFiltered?0:Number(o.discount||0),total=lineFiltered?filteredSubtotal:Number(o.total||0),salesChannel=o.salesChannel||o.sales_channel||'store',gpPercent=Number(o.gpPercent||0);return {id:o.id,external_order_number:o.externalOrderNumber||o.external_order_number||null,created_at:o.createdAt||o.created_at,subtotal,discount,item_discount:lineFiltered?0:Number(o.itemDiscount||0),bill_discount:lineFiltered?0:Number(o.billDiscount||0),total,payment_type:o.paymentType||o.payment_type||'cash',sales_channel:salesChannel,online_platform:o.onlinePlatform||null,gp_percent:gpPercent,online_net:lineFiltered&&salesChannel==='online'?total*(1-gpPercent/100):Number(o.onlineNet??o.total??0),platform_fee:lineFiltered?0:Number(o.platformFee||0),member_phone:o.memberPhone||null,received:lineFiltered?null:Number(o.received||0),change_due:lineFiltered?null:Number(o.changeDue||0),items:visibleItems};}); }
    ...
    if(path==='kds' && method==='GET') { const range=thaiDayRange(Date.now());return (await db.collection('orderItems').where('created_at','>=',range.start).where('created_at','<',range.end).orderBy('created_at','desc').limit(100).get()).docs.map(d=>({id:d.id,...d.data()})); }
    ...
    if(path==='orders' && method==='POST') {
      ...
      const lines=(data.items||[]).map(i=>{
        ...
        const original_unit_price=Number(saved?.sale_price??p.price)+customExtra,item_discount=Math.max(0,Number(i.itemDiscount)||0);
        if(!Number.isFinite(item_discount)||item_discount>original_unit_price)return err(`ส่วนลดเมนู ${localized(p).name} ไม่ถูกต้อง`);
        o.original_unit_price=original_unit_price;o.item_discount_per_unit=item_discount;
        return {product:p,quantity,options:o,original_unit_price,item_discount,unit_price:Number((original_unit_price-item_discount).toFixed(2))};
      });
      ...
      const loyalty=normalizeFirebaseLoyalty((await db.collection('settings').doc('loyalty').get()).data()||{});
      const rewardEligible=line=>loyalty.rewardMode==='all'||(loyalty.rewardMode==='category'&&loyalty.rewardCategoryKeys.includes(String(line.product.category)))||(loyalty.rewardMode==='product'&&loyalty.rewardProductIds.includes(String(line.product.id)));
      const rewardLines=loyalty.rewardType==='free_product'?lines.filter(rewardEligible):[];
      if(data.redeemFreeCup&&loyalty.rewardType==='free_product'&&!rewardLines.length)return err('ตะกร้ายังไม่มีสินค้าที่ใช้แลกรางวัลได้');
      const rewardLine=data.redeemFreeCup&&rewardLines.length?[...rewardLines].sort((a,b)=>a.unit_price-b.unit_price)[0]:null;
      const freeProductDiscount=rewardLine?(loyalty.rewardMaxPrice>0?Math.min(rewardLine.unit_price,loyalty.rewardMaxPrice):rewardLine.unit_price):0;
      const rewardDiscount=data.redeemFreeCup?(loyalty.rewardType==='fixed_discount'?loyalty.rewardDiscountAmount:freeProductDiscount):0;
      ...
      const requestedSoldAt=data.soldAt==null||data.soldAt===''?null:new Date(data.soldAt),isBackdated=Boolean(requestedSoldAt);
      if(requestedSoldAt&&(!Number.isFinite(requestedSoldAt.getTime())||requestedSoldAt.getTime()>Date.now()+60000))return err('วันเวลาขายย้อนหลังไม่ถูกต้อง');
      const subtotal=lines.reduce((n,x)=>n+x.quantity*x.original_unit_price,0),itemDiscount=lines.reduce((n,x)=>n+x.quantity*x.item_discount,0),billDiscount=Math.min(effectiveManual+rewardDiscount,Math.max(0,subtotal-itemDiscount)),discount=Math.min(itemDiscount+billDiscount,subtotal),total=subtotal-discount,id=uid(),createdAt=(requestedSoldAt||new Date()).toISOString();
      const externalOrderNumber=salesChannel==='online'?String(data.externalOrderNumber||'').trim().slice(0,40):'';
      const requestedOnlineNet=data.onlineActualReceived==null||data.onlineActualReceived===''?null:Number(data.onlineActualReceived);
      if(requestedOnlineNet!=null&&(!Number.isFinite(requestedOnlineNet)||requestedOnlineNet<0||requestedOnlineNet>total))return err('ยอดรับจริงจากแพลตฟอร์มไม่ถูกต้อง');
      const onlineNet=salesChannel==='online'?(requestedOnlineNet??Number((total*(1-gpPercent/100)).toFixed(2))):total,platformFee=salesChannel==='online'?Number((total-onlineNet).toFixed(2)):0;
      ...
      await db.runTransaction(async tx=>{
        const inventoryReads=await Promise.all(Object.entries(used).map(async([key,amount])=>{const ref=db.collection('inventory').doc(key),snap=await tx.get(ref);return {key,amount,ref,snap};}));
        ...
        if(data.redeemFreeCup&&(!memberSnap?.exists||Number(memberSnap.data().points||0)<loyalty.rewardPoints))err(`คะแนนสะสมไม่เพียงพอ ต้องใช้ ${loyalty.rewardPoints} แต้ม`);
        inventoryReads.forEach(({amount,ref,snap})=>tx.update(ref,{quantity:Number(snap.data().quantity)-amount}));
        tx.set(db.collection('orders').doc(id),{id,externalOrderNumber:externalOrderNumber||null,subtotal,discount,itemDiscount,billDiscount,total,paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,platformFee,memberPhone:data.memberPhone||null,received,changeDue,createdAt});
        lines.forEach(x=>tx.set(db.collection('orderItems').doc(),{order_id:id,product_id:String(x.product.id),name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,options_json:JSON.stringify(x.options),status:isBackdated?'completed':'pending',created_at:createdAt,createdAt}));
        if(memberSnap?.exists){const eligible=line=>loyalty.mode==='all'||(loyalty.mode==='category'&&loyalty.categoryKeys.includes(String(line.product.category)))||(loyalty.mode==='product'&&loyalty.productIds.includes(String(line.product.id))),channelCanEarn=salesChannel==='online'?loyalty.earnOnline:loyalty.earnStore,earnedBefore=channelCanEarn?lines.filter(eligible).reduce((sum,line)=>sum+line.quantity,0):0,redeemedEligible=data.redeemFreeCup&&rewardLine&&eligible(rewardLine)?1:0,oldPoints=Number(memberSnap.data().points||0),earned=Math.max(0,earnedBefore-redeemedEligible);pointsEarned=earned;memberPoints=oldPoints-(data.redeemFreeCup?loyalty.rewardPoints:0)+earned;tx.update(memberRef,{points:memberPoints});}
      });
      return {id,externalOrderNumber:externalOrderNumber||null,subtotal,discount,itemDiscount,billDiscount,total,paymentType,salesChannel,onlinePlatform,gpPercent,onlineNet,platformFee,memberPhone:data.memberPhone,memberPoints,pointsEarned,received,changeDue,createdAt,items:lines.map(x=>({name:localized(x.product).name,quantity:x.quantity,unit_price:x.unit_price,original_unit_price:x.original_unit_price,item_discount:x.item_discount,options:x.options}))};
    }
    ...
    if(path==='admin/loyalty-settings' && method==='PUT') { const settings=normalizeFirebaseLoyalty(data);if(settings.mode==='category'&&!settings.categoryKeys.length)return err('เลือกหมวดหมู่ที่ให้แต้มอย่างน้อย 1 หมวด');if(settings.mode==='product'&&!settings.productIds.length)return err('เลือกเมนูที่ให้แต้มอย่างน้อย 1 รายการ');if(!settings.earnStore&&!settings.earnOnline)return err('เลือกช่องทางสะสมแต้มอย่างน้อย 1 ช่องทาง');if(settings.rewardType==='free_product'&&settings.rewardMode==='category'&&!settings.rewardCategoryKeys.length)return err('เลือกหมวดหมู่รางวัลอย่างน้อย 1 หมวด');if(settings.rewardType==='free_product'&&settings.rewardMode==='product'&&!settings.rewardProductIds.length)return err('เลือกเมนูรางวัลอย่างน้อย 1 รายการ');await db.collection('settings').doc('loyalty').set(settings,{merge:false});return {ok:true,loyaltySettings:settings}; }
    ...
    if(path==='admin/products' && method==='POST') { const name=String(data.name||'').trim(),price=Number(data.price),defaultDiscount=Number(data.defaultDiscount||0);if(!name||!Number.isFinite(price)||price<0||!Number.isFinite(defaultDiscount)||defaultDiscount<0||defaultDiscount>price)return err('ข้อมูลเมนูไม่ถูกต้อง');const id=`prod_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,6)}`,products=await docs('products'),sortOrder=products.reduce((max,item)=>Math.max(max,Number(item.sort_order)||0),-1)+1;await db.collection('products').doc(id).set({id,name,price,default_discount:defaultDiscount,category:data.category||'other',emoji:data.emoji||'☕',image_path:data.imagePath||null,image_data:data.imageData||null,custom_options:Array.isArray(data.customOptions)?data.customOptions:[],deduct_stock:data.deductStock!==false,active:true,target_margin:.65,sort_order:sortOrder});return {id}; }
    if(path.match(/^admin\/products\/[^/]+$/) && method==='PUT') { const id=path.split('/')[2],name=String(data.name||'').trim(),price=Number(data.price),defaultDiscount=Number(data.defaultDiscount||0);if(!name||!Number.isFinite(price)||price<0||!Number.isFinite(defaultDiscount)||defaultDiscount<0||defaultDiscount>price)return err('ข้อมูลเมนูไม่ถูกต้อง');await db.collection('products').doc(id).set({name,price,default_discount:defaultDiscount,category:data.category||'other',emoji:data.emoji||'☕',image_path:data.imagePath||null,image_data:data.imageData||null,custom_options:Array.isArray(data.customOptions)?data.customOptions:[],deduct_stock:data.deductStock!==false,active:!!data.active},{merge:true});return {ok:true}; }
